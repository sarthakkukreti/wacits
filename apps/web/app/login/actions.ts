...
export async function logoutAction(): Promise<void> {
  ...
  if (token) {
    await apiSafe("/auth/logout", { method: "POST", tenant: false, sessionToken: token });
    // Otherwise this token would keep passing getSession()'s cache for up
    // to CACHE_TTL_MS even though the server-side session is now gone.
